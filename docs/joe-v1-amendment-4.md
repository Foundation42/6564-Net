...
One honest edge remained, and it was not in spawn. A device with a single reply window — the pad — admitted one subscriber, so across a game's successive lives only the first bird heard it; the rest fell in silence. That was the row's one-asker rule, not a limit of lending, and the fix was a re-subscribing pad, not a change to how capabilities pass. It is now built (§7.8): every `Poll` re-aims the pad at whoever just subscribed, so the input follows the living screen and each spawned screen takes it in turn. Probate hands an estate to a successor; A4.9 hands a supervisor's children the world it lives in; the re-subscribing pad lets each of them listen when its turn comes — and with that the whole three-screen state machine is a program, not a gap.
