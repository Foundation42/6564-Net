...
*Draft v2.3 — Entrained AI Research Institute — July 2026*

> **Changes from v2.2.** New **§7 — Peripherals: the Fabric Is the Bus**.
> Devices are actors: rows `$FF00`–`$FFFE` of mesh coordinate space become
> the **peripheral row**; device endpoints are reached through PTT
> capabilities, gated by tokens, and answer through the reply-window
> convention — no MMIO, no interrupt controller, no DMA engine distinct
> from the RBC. Later sections renumber (ISA sketch → §8, prior art → §9,
> simulator → §10, open questions → §11). §10 gains the peripheral demos;
> §11 gains discovery and off-die-bridge questions. Null cost is verified:
> the frozen v2.2 measurement table reproduces byte-identically with no
> devices attached.
...
An off-die network bridge is the planned fifth citizen of the row: a NIC is just another device, which is what makes multi-die topologies (§11) an extension rather than a redesign.
...
8. **Off-die bridges and multi-die time** — the bridge endpoint (§7.4) is where a machine leaves its die. Cross-die latency ≥ the off-chip base gives each die a lookahead horizon, which should permit deterministic parallel simulation (one die per host core) under a conservative synchronization contract; the contract itself is future work.
