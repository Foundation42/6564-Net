...
**The interface made mechanical (reference implementation).** The polyfill principle is only as strong as what can violate it, so the reference machine gives it teeth: every peripheral is reached through a single device *vtable* — `handle`, and optionally `complete` (a granted region's fence) and `push` (an unsolicited stream) — and the machine holds nothing else about a device. It cannot branch on a device's kind because it does not have one: a console, an accelerator, a display, and a pad are each just a pointer and a vtable at a row coordinate, and the machine owns only the fabric around them (the token gate, the ack, the grant/release fence, the completion record). The principle is thereby enforced *by construction* rather than by discipline — a contract an ordinary actor could not implement is one the machine could not call, because there is no place left to special-case it. The consequence is that the row is open: a new device — a real display scanning real pixels, a seed-driven or hardware pad, a GPU compute path — plugs in by supplying its own vtable, with no change to the machine and no new arm in any switch. Silicon was always an optimization; the seam it optimizes behind is now a single struct.
